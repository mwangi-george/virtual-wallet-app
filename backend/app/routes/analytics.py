...
def create_analytics_router() -> APIRouter:
    ...
    async def get_spending_summary(start_date: date = Query(date.today() - timedelta(days=90)),
        ...
        transactions = analytic_services.calculate_spending_summary(start_date, end_date, user, db)
    ...
    async def get_statement(start_date: date = Query(date.today() - timedelta(days=90)),
                            ...
                            user: User = Depends(security.get_current_user), db: Session = Depends(get_db)):
        transactions = analytic_services.fetch_transactions(start_date, end_date, transaction_type, category, user, db)
