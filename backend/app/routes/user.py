...
def create_auth_router() -> APIRouter:
    ...
    async def signup(user: CreateUser, bg_tasks: BackgroundTasks, db: Session = Depends(get_db)):
        response = user_services.create_user(user, db, bg_tasks)
    ...
    async def verify_account(token: str, request: Request,  db: Session = Depends(get_db)):
        response = user_services.verify_user(token, db)
    ...
    async def login(user_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
        token_data = user_services.login_user(user_data.username, user_data.password, db)
        return token_data
