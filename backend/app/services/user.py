...
class UserServices:
    ...
    @staticmethod
    def create_user(user: CreateUser, db: Session, bg_tasks: BackgroundTasks):

        # Check if email exists
        if security.get_user(user.email, db):
        ...
        try:
            ...
            db.add(new_user)
            db.flush()  # Flush to generate the user ID without committing
    ...
    def verify_user(token: str, db: Session):
        ...
        user = security.get_user(email, db)
    ...
    def login_user(self, email: str, password: str, db: Session) -> dict:
        user = security.authenticate_user(email, password, db)
        if not user:
